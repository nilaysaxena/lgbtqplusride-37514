...
def custom_exception_handler(exc, context):
    ...
    if response:
        if response.status_code == 403:
            response.data = {'status': status.HTTP_403_FORBIDDEN, 'message': response.data['detail']}
        if response.status_code != 403 and response.status_code != 200:
            resp_data = response.data
            if 'detail' in resp_data:
                resp_data = resp_data["detail"]
                response.data = resp_data
            response.data = {'status': response.status_code, 'message': str(resp_data)}
        return response
...
class CustomPDCFlowError(APIException):
    status_code = status.HTTP_500_INTERNAL_SERVER_ERROR
